...
struct NumTypes { enum { N = 1 + NumTypes<_Types...>::N }; };

template<typename _T>
struct NumTypes<_T> { enum { N = 1 }; };
...
public:
    ...
    value_type &at(_Int ...inds) {
        assert(NumTypes<_Int...>::N == dim());
        return at(array_helper::index_helper(int(m_shape.size()), m_shape, m_coeff, inds...));
//        int n = NumTypes<_Int...>::N;
//        if (n == dim()) return at(array_helper::index_helper(int(m_shape.size()), m_shape, m_coeff, inds...));
//        else DIE("Array::at error at ", __LINE__);
    }
...
//        else DIE("Array::at error at ", __LINE__);
    ...
    derived_type &reshape(const Shape & _shape) {
        if (alloc_size() != ::std::accumulate(_shape.begin(), _shape.end(), 1, [](int a, int b){return a*b;})) {
    ...
    void set_all(const _N & n) {
        for (int i = 0; i < alloc_size(); i++) at(i) = n;
    }
    ...
    int argmin() const {
        int ind = 0;
        value_type min = at(0);
        for (int i = 1; i < alloc_size(); i++) if (min > at(i)) {
            min = at(i);
            ind = i;
        }
        return ind;
    ...
    int argmax() const {
        int ind = 0;
        value_type max = at(0);
        for (int i = 1; i < alloc_size(); i++) if (max < at(i)) {
            max = at(i);
            ind=i;
        }
        return ind;
    ...
    T min() const {
        T min  = at(0);
        for (int i = 1; i < alloc_size(); i++) if (min > at(i)) min = at(i);
        return min;
    }
    ...
    T max() const {
        T max  = at(0);
        for (int i = 1; i < alloc_size(); i++) if (max < at(i)) max = at(i);
        return max;
    }
    ...
    T sum() const {
        T sum  = 0;
        for (int i = 0; i < alloc_size(); i++) sum  += at(i);
        return sum;
    }
    ...
    T mean() const {
        T mean = 0;
        for (int i = 0; i < alloc_size(); i++) mean += at(i);
        return T(mean / (double)alloc_size());
    }
    ...
    T var() const {
        T m = mean();
        T sum = 0;
        for (int i = 0; i < alloc_size(); i++) sum += ::std::pow(at(i)-m, 2);
        return T(sum / (double)alloc_size());
    }
    ...
    T norm()  const {
        T norm = 0;
        for (int i = 0; i < alloc_size(); i++) norm += square(at(i));
...
#define ARRAY_DEF_METHOD(name) \
    ...
    { \
        for (int i = 0; i < alloc_size(); i++) at(i) = ARRAY_METHOD(at(i), n(i)); \
        return *(derived_type*)this; \
    } \
    template<typename _N> auto name(const _N & n) \
        -> typename ::std::enable_if<!jn_is_array<_N>::value, derived_type &>::type \
    { \
        for (int i = 0; i < alloc_size(); i++) at(i) = ARRAY_METHOD(at(i), n); \
        return *(derived_type*)this; \
    }
...
struct Array : public BasicArray<_NumType, Array<_NumType>> {
     ...
     */
    template<typename _T, typename = typename ::std::enable_if<!::std::is_same<_T, int>::value>::type>
    ...
    Array(const ::std::initializer_list<int> & _shape) {
        ...
        data = new value_type[base_type::alloc_size()];
    }
    ...
    Array(const BasicArray<_V, _D> & array, const ::std::vector<::std::vector<int>> &range) {
        ...
        base_type::set_coeff();
        data.resize(base_type::alloc_size());
    ...
    void _assign(const _T &array) {
        if (base_type::alloc_size() != array.alloc_size()) realloc(array.shape());
        base_type::set_shape(array.shape());
        for (int i = 0; i < base_type::alloc_size(); i++) data[i] = array(i);
    }
     ...
     * @{
     */
    type &selfFFT()       {       ::cppsci::fft(data, data, base_type::dim(), base_type::shape(0)); return *this; }
    type &selfIFFT()      {      ::cppsci::ifft(data, data, base_type::dim(), base_type::shape(0)); return *this; }
    type &selfFFTshift()  {  ::cppsci::fftshift(data, data, base_type::dim(), base_type::shape(0)); return *this; }
    type &selfFFT1()      {      ::cppsci::fft1(data, data,                   base_type::shape(0)); return *this; }
    type &selfIFFT1()     {     ::cppsci::ifft1(data, data,                   base_type::shape(0)); return *this; }
    type &selfFFTshift1() { ::cppsci::fftshift1(data, data,                   base_type::shape(0)); return *this; }
    type &selfFFT2()      {      ::cppsci::fft2(data, data,                   base_type::shape(0)); return *this; }
    type &selfIFFT2()     {     ::cppsci::ifft2(data, data,                   base_type::shape(0)); return *this; }
    type &selfFFTshift2() { ::cppsci::fftshift2(data, data,                   base_type::shape(0)); return *this; }
    type &selfFFT3()      {      ::cppsci::fft3(data, data,                   base_type::shape(0)); return *this; }
    type &selfIFFT3()     {     ::cppsci::ifft3(data, data,                   base_type::shape(0)); return *this; }
    type &selfFFTshift3() { ::cppsci::fftshift3(data, data,                   base_type::shape(0)); return *this; }
    template<typename T>
    static type       fft(T && m) { type n(m.shape());       ::cppsci::fft(m.data, n.data, m.dim(), m.shape(0)); return ::std::move(n); }
    template<typename T>
    static type      ifft(T && m) { type n(m.shape());      ::cppsci::ifft(m.data, n.data, m.dim(), m.shape(0)); return ::std::move(n); }
    template<typename T>
    static type  fftshift(T && m) { type n(m.shape());  ::cppsci::fftshift(m.data, n.data, m.dim(), m.shape(0)); return ::std::move(n); }
    template<typename T>
    static type      fft1(T && m) { type n(m.shape());      ::cppsci::fft1(m.data, n.data,          m.shape(0)); return ::std::move(n); }
    template<typename T>
    static type     ifft1(T && m) { type n(m.shape());     ::cppsci::ifft1(m.data, n.data,          m.shape(0)); return ::std::move(n); }
    template<typename T>
    static type fftshift1(T && m) { type n(m.shape()); ::cppsci::fftshift1(m.data, n.data,          m.shape(0)); return ::std::move(n); }
    template<typename T>
    static type      fft2(T && m) { type n(m.shape());      ::cppsci::fft2(m.data, n.data,          m.shape(0)); return ::std::move(n); }
    template<typename T>
    static type     ifft2(T && m) { type n(m.shape());     ::cppsci::ifft2(m.data, n.data,          m.shape(0)); return ::std::move(n); }
    template<typename T>
    static type fftshift2(T && m) { type n(m.shape()); ::cppsci::fftshift2(m.data, n.data,          m.shape(0)); return ::std::move(n); }
    template<typename T>
    static type      fft3(T && m) { type n(m.shape());      ::cppsci::fft3(m.data, n.data,          m.shape(0)); return ::std::move(n); }
    template<typename T>
    static type     ifft3(T && m) { type n(m.shape());     ::cppsci::ifft3(m.data, n.data,          m.shape(0)); return ::std::move(n); }
    template<typename T>
    static type fftshift3(T && m) { type n(m.shape()); ::cppsci::fftshift3(m.data, n.data,          m.shape(0)); return ::std::move(n); }
    /**
     * @}
     */
    ...
    static type zero(const Shape &shape) { type m(shape); for (int i = 0; i < m.alloc_size(); i++) m(i) = 1; return ::std::move(m); }

    /** Return an array with all the elements being 1 */
    static type ones(const Shape &shape) { type m(shape); for (int i = 0; i < m.alloc_size(); i++) m(i) = 1; return ::std::move(m); }

    /** Return an array with all the elements being value */
    template<typename _V>
    static type constants(const Shape &shape, const _V &value) {
        type m(shape);
        for (int i = 0; i < m.alloc_size(); i++) m(i) = 1;
        return ::std::move(m);
    }
    /**
     * @}
...
#define ARRAY_DEF_METHOD(name) \
    ...
    -> typename ::std::enable_if<jn_is_array<_M>::value && jn_is_array<_N>::value, type>::type{ \
        type r(m.shape()); \
        for (int i = 0; i < r.alloc_size(); i++) r(i) = ARRAY_METHOD(m(i), n(i)); \
        return ::std::move(r);\
    }\
    template<typename _M, typename _N> static auto name(const _M & m, const _N & n) \
    -> typename ::std::enable_if<jn_is_array<_M>::value && !jn_is_array<_N>::value, type>::type{ \
        type r(m.shape()); \
        for (int i = 0; i < r.alloc_size(); i++) r(i) = ARRAY_METHOD(m(i), n); \
        return ::std::move(r);\
    }\
    template<typename _M, typename _N> static auto name(const _M & m, const _N & n) \
    -> typename ::std::enable_if<!jn_is_array<_M>::value && jn_is_array<_N>::value, type>::type{ \
        type r(n.shape()); \
        for (int i = 0; i < r.alloc_size(); i++) r(i) = ARRAY_METHOD(m, n(i)); \
        return ::std::move(r);\
    }
...
#undef ARRAY_DEF_METHOD
    ...
    static type map(_F && f, _M1 && m1) {
        type r(m1.shape()); 
        for (int i = 0; i < r.alloc_size(); i++) r[i] = f(m1[i]); 
        return ::std::move(r);
    }
    ...
    static type map(_F && f, _M1 && m1, _M2 && m2) {
        ...
        type r(m1.shape()); 
        for (int i = 0; i < r.alloc_size(); i++) r[i] = f(m1[i], m2[i]); 
        return ::std::move(r);
    }
    ...
    static type apply(_M && m1, _F && f) {
        type m2(m1.shape()); 
        for (int i = 0; i < m2.alloc_size(); i++) m2(i) = f(m1(i)); 
        return ::std::move(m2);
    }
    ...
    template<typename _M> static type conj   (_M && m1) {
        type m2(m1.shape()); 
        for (int i = 0; i < m2.alloc_size(); i++) m2(i) = ::std::conj(m1(i)); 
        return ::std::move(m2);
    }
    ...
    template<typename _M> static type real   (_M && m1) {
        type m2(m1.shape()); 
        for (int i = 0; i < m2.alloc_size(); i++) m2(i) = ::std::real(m1(i)); 
        return ::std::move(m2);
    }
    ...
    template<typename _M> static type imag   (_M && m1) {
        type m2(m1.shape()); 
        for (int i = 0; i < m2.alloc_size(); i++) m2(i) = ::std::imag(m1(i)); 
        return ::std::move(m2);
    }
    ...
    template<typename _M> static type square (_M && m1) {
        type m2(m1.shape()); 
        for (int i = 0; i < m2.alloc_size(); i++) m2(i) = m1(i)*m1(i); 
        return ::std::move(m2);
    }
    ...
    template<typename _M> static type exp    (_M && m1) {
        type m2(m1.shape()); 
        for (int i = 0; i < m2.alloc_size(); i++) m2(i) = ::std::exp(m1(i)); 
        return ::std::move(m2);
    }
    ...
    template<typename _M> static type log    (_M && m1) {
        type m2(m1.shape()); 
        for (int i = 0; i < m2.alloc_size(); i++) m2(i) = ::std::log(m1(i)); 
        return ::std::move(m2);
    }
    ...
    template<typename _M> static type sqrt   (_M && m1) {
        type m2(m1.shape()); 
        for (int i = 0; i < m2.alloc_size(); i++) m2(i) = ::std::sqrt(m1(i)); 
        return ::std::move(m2);
    }
    ...
    template<typename _M> static type logicalNot(_M && m1) {
        type m2(m1.shape()); 
        for (int i = 0; i < m2.alloc_size(); i++) m2(i) = !m1(i); 
        return ::std::move(m2);
    }
    ...
    template<typename _M> static type abs(const _M & m1) {
        type m2(m1.shape());
        for (int i = 0; i < m2.alloc_size(); i++) m2(i) = ::std::abs(m1(i));
        return ::std::move(m2);
    }
    ...
    template<typename _M> static type diff(const _M & m1) {
        type m2({m1.size()-1});
        for (int i = 0; i < m2.alloc_size(); i++) m2(i) = m1(i+1)-m1(i);
        return ::std::move(m2);
    }
...
public:
    ...
    void init(_Array && array, const ::std::vector<::std::vector<int>> &range) {
        ...
        for (int i = 0; i < dim; i++) base_type::m_shape[i] = range[i][1] - range[i][0];
        base_type::set_coeff();
        m_data.resize(base_type::alloc_size());
