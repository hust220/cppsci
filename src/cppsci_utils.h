...
template<typename T> Arrayi unravel_index(const ::std::initializer_list<T> && values, const Shape &shape) { return unravel_index_helper(values, shape); }
template<typename T> Arrayi unravel_index(T && values, const Shape &shape) { return unravel_index_helper(values, shape); }
...
struct ArrayWindowRoller {
    ...
    //using win_type = SubArray<value_type>;

    //SubArray<value_type> win;
    ...
    ArrayWindowRoller(const Shape & _shape, const Shape & _winSize, const Shape & _step = {1, 1, 1}) {
        ...
        //dim = array->dim();
        range.resize(dim);
        for (int i = 0; i < dim; i++) range[i] = {0, winSize[i]};
    }
