...
#ifndef ERROR_REPORT
#define ERROR_REPORT(what) {throw cppsci_error(what,__FILE__,__LINE__);}
#endif
...
//
class cppsci_error { // : public ::std::exception
public:
    explicit cppsci_error( const char* _what_arg ,const char* _file, int _line) : what_arg(_what_arg),file(_file),line(_line) {
        ::std::cout<<"Errors encountered, "<<what_arg<<" ,"<<file<<" ,"<<line<<::std::endl;
    }
    explicit cppsci_error( const ::std::string& _what_arg ,const char* _file, int _line) : what_arg(_what_arg.c_str()),file(_file),line(_line) {
        ::std::cout<<"Errors encountered, "<<what_arg<<" ,"<<file<<" ,"<<line<<::std::endl;
    }
    ~cppsci_error(){}
