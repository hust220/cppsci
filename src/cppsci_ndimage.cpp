...
static void spline_filter1d(double *ln, int len, int order) {
    ...
    if (order < 0 or order > 5) throw "error";
...
void spline_filter(Arrayd &input, int order) {
    ...
    for (int i = 0; i < nd; i++) {
        ...
        for (int j = 0; j < temp.shape(0); j++) {
            spline_filter1d(temp.data + j*li, li, order);
        }
        m = 0;
        ARRAY_EACH(input.shape(), ind) {
            int n = 0;
...
//    MPI_LOG << "filtered:\n" << filtered << ::std::endl;
    ...
    for (int i = 0; i < nd; i++) {
        output_shape[i] = int(::std::round(input.shape(i) * _zoom));
        ...
        int order, int mode, double cval)
{
    ...
    double ***splvals = NULL;
    NI_Iterator io;
    double *zooms = (zoom_ar ? zoom_ar->data : NULL);
    double *shifts = (shift_ar ? shift_ar->data : NULL);
    int rank = 0, qq;
    ...
    pi = reinterpret_cast<char *>(input.data);
    po = reinterpret_cast<char *>(output.data);
        ...
        Arrayd &output, int order, int mode, double cval)
{
    ...
    npy_intp idimensions[MAXDIM], istrides[MAXDIM];
    NI_Iterator io, ic;
    double *matrix = matrix_ar.data;
    double *shift = shift_ar.data;
...
//        pc = (void *)(PyArray_DATA(coordinates));
        pc = reinterpret_cast<char *>(coordinates.data);
    }
        ...
        int order, ::std::string mode, double cval, bool prefilter)
{
    ...
    Arrayd output(output_shape);
    basic_geometric_transform(filtered, NULL, NULL, coordinates, Arrayd{}, Arrayd{}, output, order, mode_code, cval);
    return ::std::move(output);
